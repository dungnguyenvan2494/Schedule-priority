...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread)
    list_insert_ordered (&ready_list, &cur->elem, compare_priority, 0);	// sắp xếp các thread theo mức độ ưu tiên trước khi đưa vảo ready_list
...
void
thread_set_priority (int new_priority)  // Thay vì set priority cho thread hiện tại thì sẽ lưu priority cũ vào priorites[] rồi gán priority mới cho thread hiện tại
{
...
init_thread (struct thread *t, const char *name, int priority)
{
 ...
 /* Tạo thread rỗng với 1 mức ưu tiên*/
...
/*Sorts the ready_list present in thread.c*/
 void sort_ready_list(void) // Sắp xếp các thread ready theo compare_priority
{
